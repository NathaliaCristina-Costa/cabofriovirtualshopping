          ...
          <div class="card">
            <div class="card-body">
              ...
              <table class="table datatable">
                ...
                <tbody>
                    <?php
                      $dados = $f->listarFretesCentro();
                      if (count($dados) > 0) {
                        for ($i=0; $i < count($dados); $i++) {  
                          echo '<tr>'; 
                            foreach ($dados[$i] as $key => $value) {
                                if ($key!='idFreteCentro') {
                                  echo '<td>'.$value.'</td>';
                                }
                            }                     
                    ?>
                    <td><a href="frete.php?id=<?php echo $dados[$i]['idFreteCentro']; ?>" class="btn btn-dark btn-circle"><i class="bi bi-eye"></i></a></td>
                  <?php
                        echo '</tr>';
                      }
                    }
